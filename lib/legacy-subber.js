...
class Subber {
  ...
  read (source, query) {
    ...
    if (this.runningContext) {
      subberRegisterQuery.call(this, this.runningContext, source, query, value)
    }
    return value
  }
...
function subberRegisterQuery (context, source, query, value) {
...
function subberNotify (source) {
  if (this.runningContext) {
    ...
    // Should we que the notification?
    return
  }
...
function cleanupContext (context) {
  if (context) flushBy(context.sourceCursors, cursorRemoveSubscriber)
}

function cursorRemoveSubscriber (cursor) {
  return cursor.removeSubscriber()
}
