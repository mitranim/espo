...
const {isReactiveSource} = require('./reactives')
...
class Subber {
  ...
  read (source, query) {
    ...
    if (this.runningContext) {
      attune.call(this, this.runningContext, source, query, value)
    }
    return value
  }
...
function attune (context, source, query, value) {
  let sourceCursor = findSourceCursor(context, source)

  if (!sourceCursor) {
    // New function reference to avoid accidental deduplication
    sourceCursor = SourceCursor(source, source.addSubscriber(this._onNotify.bind(this)))
